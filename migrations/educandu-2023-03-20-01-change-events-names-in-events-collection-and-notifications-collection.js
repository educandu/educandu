export default class Educandu_2023_03_20_01_change_events_names_in_events_collection_and_notifications_collection {
  ...
  async up() {
    ...
    await this.db.collection('events').updateMany({ type: 'revision-created' }, { $rename: { 'eventParams.revisionId': 'eventParams.documentRevisionId' } });
  ...
  async down() {
    ...
    await this.db.collection('events').updateMany({ type: 'revision-created' }, { $rename: { 'eventParams.documentRevisionId': 'eventParams.revisionId' } });
