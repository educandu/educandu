export default class Educandu_2023_01_13_01_remove_midi_from_abc_notation_plugin {
  constructor(db) {
    this.db = db;
  }
  ...
  async collectionUp(collectionName) {
    const result = await this.db.collection(collectionName).updateMany(
      ...
      {
        $unset: {
          'sections.$[sectionElement].content.displayMidi': null
        }
      },
      {
        arrayFilters: [
  ...
  async collectionDown(collectionName) {
    const result = await this.db.collection(collectionName).updateMany(
      ...
      {
        $set: {
          'sections.$[sectionElement].content.displayMidi': false
        }
      },
      {
        arrayFilters: [
