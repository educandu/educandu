export default class Educandu_2023_03_21_01_change_events_names_in_events_collection_and_notifications_collection {
  constructor(db) {
    this.db = db;
  }

  async up() {
    await this.db.collection('events').updateMany({ type: 'revision-created' }, { $rename: { 'params.revisionId': 'params.documentRevisionId' } });
    ...
    await this.db.collection('notifications').updateMany({ eventType: 'revision-created' }, { $rename: { 'eventParams.revisionId': 'eventParams.documentRevisionId' } });
  ...
  async down() {
    await this.db.collection('events').updateMany({ type: 'document-revision-created' }, { $rename: { 'params.documentRevisionId': 'params.revisionId' } });
    ...
    await this.db.collection('notifications').updateMany({ eventType: 'document-revision-created' }, { $rename: { 'eventParams.documentRevisionId': 'eventParams.revisionId' } });
