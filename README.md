...
The output of this repository is an npm package (`@educandu/educandu`).
 ...
 | externalAccountProviders | Configuration for SAML authentication | `{ key: <string>, displayName: <string>, loginUrl: <string>, logoUrl: <string>, expiryTimeoutInDays: <number> }` | no, however if provided, all fields except `logoUrl` and `expiryTimeoutInDays` are mandatory |
