...
function ResourceSearchBar({ isLoading, initialSearchParams, onSearch }) {
  ...
  const [selectedResourceType, setSelectedResourceType] = useState(initialSearchParams.searchResourceType);
  ...
  useEffect(() => {
    setSelectedResourceType(initialSearchParams.searchResourceType);
  ...
  return (
    <div className="ResourceSearchBar">
      ...
      <RadioGroup value={selectedResourceType} disabled={isLoading} onChange={handleSelectedResourceTypeChange}>
        <RadioButton value={MEDIA_SEARCH_RESOURCE_TYPE.audio}>{t(`common:resourceType_${MEDIA_SEARCH_RESOURCE_TYPE.audio}`)}</RadioButton>
